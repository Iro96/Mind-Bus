...
def test_reflector_summarizes_file_system_results():
    state = _base_state()
    state["tool_results"] = [
        {"tool": "file_system", "action": "read_text", "path": "/tmp/demo.txt", "success": True, "chars": 4},
        {"tool": "file_system", "action": "list_dir", "path": "/tmp", "success": True, "count": 2, "truncated": False},
        {"tool": "file_system", "action": "write_text", "path": "/tmp/demo.txt", "success": True, "chars": 4},
    ]
    state["last_tool_calls"] = [
        {"name": "file_system", "args": {"action": "read_text", "path": "/tmp/demo.txt"}},
        {"name": "file_system", "args": {"action": "list_dir", "path": "/tmp"}},
        {"name": "file_system", "args": {"action": "write_text", "path": "/tmp/demo.txt"}},
    ]

    updated = reflector(state)

    assert "file_system read_text" in updated["response_context"]
    assert "file_system list_dir" in updated["response_context"]
    assert "file_system write_text" in updated["response_context"]
    assert updated["reflection"]["status"] == "ready"


def test_reflector_marks_file_system_failures_non_retryable():
    state = _base_state()
    state["tool_results"] = [
        {
            "tool": "file_system",
            ...
            "path": "/tmp/demo.txt",
            "success": False,
            "error": "file system write is disabled",
        }
    ]
    state["last_tool_calls"] = [{"name": "file_system", "args": {"action": "write_text", "path": "/tmp/demo.txt"}}]
